...
Image caching has been configured to automatically occur during the creation of the {project} VM while executing `minishift start`.
After the completion of `minishift start`, caching occurs in a background process, the first time you use the xref:../command-ref/minishift_start#[`minishift start`] command.
...
You can again enable the caching of the OpenShift images by setting `image-caching` to `true`.
