...
[IMPORTANT]
====
This only works with the CentOS/RHEL based image and is currently not supported on KVM as the driver plugin relies on the DHCP offer to determine the IP address.
====
