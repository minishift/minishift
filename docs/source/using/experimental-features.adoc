...
For example, the following command will pass the `--service-catalog` flag to `oc cluster up` to provision the OpenShift link:https://docs.okd.io/latest/architecture/service_catalog/index.html[*service catalog*]:
