...
For example, following command will pass the `--service-catalog` flag to `oc cluster up` to provision the OpenShift link:https://docs.openshift.org/latest/architecture/service_catalog/index.html[*service catalog*]:
