...
func getServiceURL(serviceSpecs []openshift.ServiceSpec, ip string) string {
	...
	if len(namespaceList) > 1 {
		...
		atexit.ExitWithMessage(1, fmt.Sprintf("Service %s exists in (%s) namespaces, you need to chose a specific namespace using -n <namespace_name>", service, namespaces))
	}
