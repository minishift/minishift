...
func (m *MachineConfig) CacheMinikubeISOFromURL() error {
	...
	if response.StatusCode != http.StatusOK {
		return fmt.Errorf("Received %d response from %s while trying to download minishift.iso", response.StatusCode, m.MinikubeISO)
	}
...
func createHost(api libmachine.API, config MachineConfig) (*host.Host, error) {
	...
	if err := api.Create(h); err != nil {
		...
		return nil, fmt.Errorf("Error creating store. %s", err)
	}
