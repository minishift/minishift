...
To do so you can specify the optional OpenShift-Version metadata field. The semantics are as follow:
...
NOTE: If the metadata field OpenShift-Version is not specified in the add-on header, the add-on is applied against any version of OpenShift.

NOTE: OpenShift-Version should be in form of <major>.<minor>.<patch>
