...
[[enabling-experimental-openshift-features]]
== Enabling Experimental OpenShift Features

By default, Minishift does not expose all `oc cluster up` flags in the CLI, either because certain features are not considered stable or because Minishift does not yet support the latest options in `oc cluster up`.

However, if you want to get early access to some upcoming features and experiment, you can enable some of those in Minishift.
...
Currently this environment variable controls additional options only for the xref:../command-ref/minishift_start.adoc#[`minishift start`] command.
The additional options are related to  xref:../openshift/openshift-client-binary.adoc#[`oc cluster up`].

When you set the environment variable, you get access to the following flags:
...
Enables passing any extra flags to `oc cluster up`.
