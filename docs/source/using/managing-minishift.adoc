...
By default, Minishift does not expose all xref:../openshift/openshift-client-binary.adoc#[`oc cluster up`] flags in the CLI, either because certain features are not considered stable or because Minishift does not yet support the latest options in `oc cluster up`.

You can set the environment variable to enable the following options for the xref:../command-ref/minishift_start.adoc#[`minishift start`] command:
