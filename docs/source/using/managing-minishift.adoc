...
The xref:../command-ref/minishift_start.adoc#minishift-start[`minishift start`] command creates and configures the Minishift VM and provisions a local, single-node OpenShift cluster within the Minishift VM.
...
The xref:../command-ref/minishift_stop.adoc#minishift-stop[`minishift stop`] command stops your OpenShift cluster and shuts down the Minishift VM, but preserves the OpenShift cluster state.
...
The xref:../command-ref/minishift_delete.adoc#minishift-delete[`minishift delete`] command deletes the OpenShift cluster, and also shuts down and deletes the Minishift VM.
...
For example, the `vm-driver` flag of the xref:../command-ref/minishift_start.adoc#minishift-start[`minishift start`] command becomes `MINISHIFT_vm-driver`.
...
NOTE: Persistent configuration can only be applied to the set of supported configuration options that are listed in the synopsis of the xref:../command-ref/minishift_config.adoc#minishift-config[`minishift config`] sub-command.
...
The easiest way to change a persistent configuration option is with the xref:../command-ref/minishift_config_set.adoc#minishift-config-set[`minishift config set`] sub-command.
For example:
...
To view all persistent configuration values, you can use the xref:../command-ref/minishift_config_view.adoc#minishift-config-view[`minishift config view`] sub-command:
...
Alternatively, you can display a single value with the xref:../command-ref/minishift_config_get.adoc#minishift-config-get[`minishift config get`] sub-command:
...
To remove a persistent configuration option, you can use the xref:../command-ref/minishift_config_unset.adoc#minishift-config-unset[`minishift config unset`] sub-command.
For example:
...
To use driver-specific options, make sure to export the variable as defined in its driver documentation before running xref:../command-ref/minishift_start.adoc#minishift-start[`minishift start`].
...
This feature is considered experimental and needs to be explicitly enabled using the xref:../command-ref/minishift_config_set#minishift-config-set[`minishift config set`] command:

----
$ minishift config set image-caching true
----

Once enabled, caching occurs transparently, in a background process, the first time you use the xref:../command-ref/minishift_start#minishift-ssh[`minishift start`] command.
...
You can disable the caching of the OpenShift images by setting `image-caching` to `false` or removing the setting altogether using xref:../command-ref/minishift_config_unset#minishift-config-unset[`minishift config unset`]:
...
The location of the persistent data is determined in the `host-pv-dir` flag of the xref:../command-ref/minishift_start.adoc#minishift-start[`minishift start`] command and defaults to *_/var/lib/minishift/openshift.local.pv_* on the Minishift VM.
...
You can list all Minishift-compatible OpenShift versions with the xref:../command-ref/minishift_openshift_version_list.adoc#minishift-openshift-version-list[`minishift openshift version list`] command.
====

[[networking]]
== Networking

The Minishift VM is exposed to the host system with a host-only IP address that can be obtained with the xref:../command-ref/minishift_ip.adoc#minishift-ip[`minishift ip`] command.
