...
Feature: Basic
  ...
  Scenario: User can deploy a Ruby example application
   Given Minishift has state "Running"
    ...
    Then with up to "10" retries with wait period of "50ms" the "status code" of HTTP request to "/" of service "ruby-ex" in namespace "ruby" is equal to "200"
     And with up to "10" retries with wait period of "50ms" the "body" of HTTP request to "/" of service "ruby-ex" in namespace "ruby" contains "Welcome to your Ruby application on OpenShift"
