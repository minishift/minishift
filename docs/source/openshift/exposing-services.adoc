...
If you are deploying a Web application, the most common way to expose it is by a link:https://docs.okd.io/latest/dev_guide/routes.html[route].
...
In case the service you want to expose is not HTTP based, you can create a link:https://docs.okd.io/latest/architecture/core_concepts/pods_and_services.html#service-nodeport[*NodePort*] service.
In this case, each OpenShift node will proxy that port into your service.
To access this port on your {project} VM, you need to configure an link:https://docs.okd.io/latest/dev_guide/expose_service/expose_internal_ip_nodeport.html[Ingress IP] using `oc expose` with the parameter `type=LoadBalancer`.
